...
namespace ps
{
    ...
    void Segments::SetSegmentsGrid(double seg_size)
    {
        ...
        for (int i = P->grid_curve_calc; i < grid_count_x - P->grid_curve_calc; i++)
        {
            double xai = (i + 0.5) * grid_x_size + P->area_beg;
            gxa[i - P->grid_curve_calc] = xai;
        }
        for (int i = P->grid_curve_calc; i < grid_count_z - P->grid_curve_calc; i++)
        {
    ...
    void Segments::PrintGrid(int num)
    {
        std::string output("x,z,burn_radius,ok_count,burn_count");
        // output += "#base_particles " + std::to_string(P->base_particles);
        //  output += "x,z,burn_radius,ok_count,burn_count";

        for (const auto &seg : grid)
        {
            {

                double x = seg.seg_start_x + grid_x_size / 2;
                double z = seg.seg_start_z + grid_z_size / 2;
                output += fmt::format("\n{},{},{},{},{}", x, z, seg.br, seg.ok_size, seg.b_size);
            }
        }
    ...
    {
        tbb::parallel_for_each(grid.begin(), grid.end(), [this](Segment &seg) {
            if (!(seg.burn_list.empty())) {
                DoSegment(seg);
            } 
            });
    ...
    void Segments::StepParticles()
    {
        ...
        tbb::parallel_for_each(all_list.begin(), all_list.end(), [this](Particle &p)
            {
    ...
    void Segments::MoveParticle(Particle &p)
    {
        ...
        if (p.z < 0 || p.z >= P->area_height || p.x > P->area_end || p.x < P->area_beg)
            p.state = Particle::State::DIED;
    }
    ...
    void Segments::StepParticle(Particle &p)
    {
        ...
        if (p.state == Particle::State::WARM &&
    ...
    {
        tbb::parallel_for_each(segment.ok_list.begin(), segment.ok_list.end(), [this](SegPointOk &p) { 
    ...
    {
        auto erase_it = std::remove_if(all_list.begin(), all_list.end(), [](Particle &p) { 
            return p.state == Particle::State::DIED; 
        });       

        // tbb::parallel_sort(all_list.begin(), all_list.end(), [](Particle &a, Particle b) { 
        //     return a.state != Particle::State::DIED && b.state == Particle::State::DIED; 
        // });

        // auto erase_it = std::find_if(all_list.begin(), all_list.end(), [](Particle &p) {
        //     return p.state == Particle::State::DIED;
        // });
    ...
    {
        tbb::parallel_for_each(grid.begin(), grid.end(), [this, g](Segment &seg) {
            seg.br = P->burn_radius_cross;
            seg.br2 = P->burn_radius_2_cross;
            seg.c_ok = seg.c_b = seg.curvature = 0;//seg.br = seg.br2 = 0;
            if (seg.b_size == 0) return;
            //if (seg.x >= g && seg.x < grid_count_x - g && seg.z >= g && seg.z < grid_count_z - g) 
            {
                    ...
                    for (int xi = seg_x_start; xi < seg_x_end; xi++) {
                        for (int zi = seg_z_start; zi < seg_z_end; zi++) {
                            if (!grids(xi, zi).ok_list.empty()) {
                                grids(xi, zi).burn_list.emplace_back(bp, seg.br2);
                            }
                        }
                    }
                }
